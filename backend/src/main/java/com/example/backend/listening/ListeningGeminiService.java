...
public class ListeningGeminiService {
  ...
  public List<GeneratedDialog> generateDialogs(int count) {
    if (count <= 0)
      count = 4;
    String prompt = "You are an expert creator of short multi-turn dialogue listening exercises (TOEIC style)."
        + "\nGenerate " + count
        + " unique dialogs. Each dialog must be a short natural conversation between multiple speakers (2 or 3 different speakers)."
        + "\nStructure each dialog as 2-6 short speaker lines, each line starting with a speaker label (e.g. 'Man:', 'Woman:', 'Student:', 'Announcer:')."
        + "\nFor each dialog, provide exactly ONE question that tests listening comprehension (detail, main idea, or inference). Do NOT output problem-type labels such as 'Paraphrase', 'Fill the blank', or any other category tag."
        + "\nProvide exactly three answer choices (A, B, C). One choice must be correct."
